...
void draw() {
    ...
    for (int i = 0; i < floor(iterationFrames / speed) || skipGeneration; i++) { 
        // Keep simulating iterations without drawing anything, and only simulate a frame if, when the speed is above 1, some number of frames have passed
        population.frame();
    }
...
void drawUI() {
    ...
    if (selectedAgent != null) {
        ...
        String output = "top score: " + population.topScore + " (gen " + population.topScoreGeneration + ")\n" +
            "speed: " + (1 / speed) + "\n" +
            "generation: " + (generationCount + 1) + "\n" + 
            "generation progress: " + generationElapsed + "/" + generationLength + "\n" + 
...
void drawProgress() {
    ...
    text(generationIteration + "/" + generationSpeed, width / 2, height / 2 + 64);
}